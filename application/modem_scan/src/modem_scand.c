...
{
	char out[16384];
	char *argv[] = { "uci", "-q", "show", "xmodem", NULL };
	char *save = NULL, *line;
	int rc;
...
static void get_slot_option(const char *slot, const char *opt, char *out, size_t len)
{
	...
	if (find_slot_section(slot, section, sizeof(section)) != 0)
		return;
	snprintf(key, sizeof(key), "xmodem.%s.%s", section, opt);
	uci_get(key, out, len);
}
...
static int at_command(const char *port, const char *cmd, int fast, int timeout, char *out, size_t out_len)
{
	...
	time_t start;
	const char *script =
		". /usr/share/xmodem/modem_util.sh; "
		"if [ \"$1\" = 1 ]; then "
		"fastat \"$2\" \"$3\"; "
		"else at \"$2\" \"$3\"; fi";
	...
	if (pid == 0) {
		...
		close(pipefd[0]);
		close(pipefd[1]);
		execl("/bin/sh", "sh", "-c", script, "xmodem-at", fast ? "1" : "0", port, cmd, NULL);
		_exit(127);
	}
...
static void normalize_model_name(char *name, size_t len)
{
	...
	if (strstr(lower, "l850")) snprintf(name, len, "l850-gl");
...
static int match_profile_by_id(const char *slot_type, const char *vid, const char *pid, struct modem_profile *profile)
{
	...
	json_object_object_foreach(type_obj, key, val) {
		...
		char ids[128], *save = NULL, *tok;
		snprintf(ids, sizeof(ids), "%s", cfg_id);
		for (tok = strtok_r(ids, " ", &save); tok; tok = strtok_r(NULL, " ", &save)) {
			if (!strcmp(tok, id))
...
{
	char *argv[] = { "/usr/share/xmodem/modem_hook.sh", (char *)section, "post_init", NULL };
	run_exec(argv, 60);
}

static void reload_network(void)
{
	char *argv[] = { "/etc/init.d/xmodem_network", "reload", NULL };
	run_exec(argv, 60);
}
...
static int add_modem(const char *slot, const char *slot_type)
{
	...
	snprintf(key, sizeof(key), "xmodem.%s.fixed_device", section);
	...
	snprintf(key, sizeof(key), "xmodem.%s", section);
	existed = !uci_get(key, existing, sizeof(existing)) && existing[0];
	if (existed) {
		snprintf(key, sizeof(key), "xmodem.%s.network", section);
		uci_get(key, orig_network, sizeof(orig_network));
		snprintf(key, sizeof(key), "xmodem.%s.at_port", section);
		uci_get(key, orig_at, sizeof(orig_at));
		snprintf(key, sizeof(key), "xmodem.%s.state", section);
		uci_get(key, orig_state, sizeof(orig_state));
		snprintf(key, sizeof(key), "xmodem.%s.name", section);
		uci_get(key, orig_name, sizeof(orig_name));
		snprintf(key, sizeof(key), "xmodem.%s.modes", section); uci_del(key);
		snprintf(key, sizeof(key), "xmodem.%s.valid_at_ports", section); uci_del(key);
		snprintf(key, sizeof(key), "xmodem.%s.tty_devices", section); uci_del(key);
		snprintf(key, sizeof(key), "xmodem.%s.net_devices", section); uci_del(key);
		snprintf(key, sizeof(key), "xmodem.%s.ports", section); uci_del(key);
		snprintf(key, sizeof(key), "xmodem.%s.state", section); uci_set(key, "enabled");
	} else {
		...
		uci_get("xmodem.main.modem_count", modem_count_s, sizeof(modem_count_s));
		if (modem_count_s[0])
			modem_count = atoi(modem_count_s);
		modem_count++;
		snprintf(modem_count_s, sizeof(modem_count_s), "%d", modem_count);
		uci_set("xmodem.main.modem_count", modem_count_s);
		snprintf(key, sizeof(key), "xmodem.%s", section); uci_set(key, "modem-device");
		if (default_alias[0]) { snprintf(key, sizeof(key), "xmodem.%s.alias", section); uci_set(key, default_alias); }
		if (led_script[0]) { snprintf(key, sizeof(key), "xmodem.%s.led_script", section); uci_set(key, led_script); }
		snprintf(metric, sizeof(metric), "%d", modem_count + 10);
		if (default_metric[0])
			snprintf(metric, sizeof(metric), "%s", default_metric);
		snprintf(key, sizeof(key), "xmodem.%s.path", section); uci_set(key, res.modem_path);
		snprintf(key, sizeof(key), "xmodem.%s.data_interface", section); uci_set(key, slot_type);
		snprintf(key, sizeof(key), "xmodem.%s.enable_dial", section); uci_set(key, "1");
		snprintf(key, sizeof(key), "xmodem.%s.soft_reboot", section); uci_set(key, "1");
		snprintf(key, sizeof(key), "xmodem.%s.extend_prefix", section); uci_set(key, "1");
		snprintf(key, sizeof(key), "xmodem.%s.pdp_type", section); uci_set(key, "ipv4v6");
		snprintf(key, sizeof(key), "xmodem.%s.state", section); uci_set(key, "enabled");
		snprintf(key, sizeof(key), "xmodem.%s.metric", section); uci_set(key, metric);
	}

	snprintf(key, sizeof(key), "xmodem.%s.name", section); uci_set(key, profile.name);
	snprintf(key, sizeof(key), "xmodem.%s.network", section); uci_set(key, net_join);
	snprintf(key, sizeof(key), "xmodem.%s.manufacturer", section); uci_set(key, profile.manufacturer);
	snprintf(key, sizeof(key), "xmodem.%s.platform", section); uci_set(key, profile.platform);
	snprintf(key, sizeof(key), "xmodem.%s.suggest_pdp_index", section); uci_set(key, profile.pdp_index);
	if (profile.wcdma_band[0]) { snprintf(key, sizeof(key), "xmodem.%s.wcdma_band", section); uci_set(key, profile.wcdma_band); }
	if (profile.lte_band[0]) { snprintf(key, sizeof(key), "xmodem.%s.lte_band", section); uci_set(key, profile.lte_band); }
	if (profile.nsa_band[0]) { snprintf(key, sizeof(key), "xmodem.%s.nsa_band", section); uci_set(key, profile.nsa_band); }
	if (profile.sa_band[0]) { snprintf(key, sizeof(key), "xmodem.%s.sa_band", section); uci_set(key, profile.sa_band); }
	for (size_t i = 0; i < profile.modes.len; i++) {
		snprintf(key, sizeof(key), "xmodem.%s.modes", section);
		uci_add_list(key, profile.modes.items[i]);
	}
	for (size_t i = 0; i < res.valid_at_ports.len; i++) {
		snprintf(key, sizeof(key), "xmodem.%s.valid_at_ports", section);
	...
	for (size_t i = 0; i < res.at_ports.len; i++) {
		snprintf(key, sizeof(key), "xmodem.%s.ports", section);
		uci_add_list(key, res.at_ports.items[i]);
	}
	if (res.option_driver) {
		snprintf(key, sizeof(key), "xmodem.%s.option_driver", section);
		uci_set(key, "1");
	}
	uci_commit("xmodem");
	...
	{
		char rundir[256];
		snprintf(rundir, sizeof(rundir), XMODEM_RUN_DIR "/%s_dir", section);
		mkdir(XMODEM_RUN_DIR, 0755);
		mkdir(rundir, 0755);
	}
	exec_post_init(section);
...
static void remove_modem(const char *section)
{
	...
	if (uci_get(key, existing, sizeof(existing)) || !existing[0])
		return;
	pthread_mutex_lock(&uci_lock);
	uci_get("xmodem.main.modem_count", count_s, sizeof(count_s));
	if (count_s[0])
		count = atoi(count_s);
	if (count > 0)
		count--;
	snprintf(count_s, sizeof(count_s), "%d", count);
	uci_set("xmodem.main.modem_count", count_s);
	snprintf(key, sizeof(key), "xmodem.%s", section); uci_del(key);
...
static void disable_slot(const char *slot)
{
	...
	section_from_slot(slot, section, sizeof(section));
	pthread_mutex_lock(&uci_lock);
	snprintf(key, sizeof(key), "xmodem.%s=1", section);
...
static int setup_socket(void)
{
	...
	mkdir(XMODEM_RUN_DIR, 0755);
	unlink(XMODEM_SCAND_SOCKET);
	fd = socket(AF_UNIX, SOCK_STREAM, 0);
	if (fd < 0)
		return -1;
	memset(&addr, 0, sizeof(addr));
	addr.sun_family = AF_UNIX;
	snprintf(addr.sun_path, sizeof(addr.sun_path), "%s", XMODEM_SCAND_SOCKET);
	if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		close(fd);
		return -1;
	}
	chmod(XMODEM_SCAND_SOCKET, 0666);
	if (listen(fd, 32) < 0) {
		close(fd);
		return -1;
	}
...
static void load_config_defaults(void)
{
	...
	if (!uci_get("xmodem.main.scan_workers", val, sizeof(val)) && atoi(val) > 0)
		g.scan_workers = atoi(val);
	if (!uci_get("xmodem.main.at_probe_workers", val, sizeof(val)) && atoi(val) > 0)
		g.at_probe_workers = atoi(val);
	if (!uci_get("xmodem.main.at_timeout_fast", val, sizeof(val)) && atoi(val) > 0)
		g.at_timeout_fast = atoi(val);
	if (!uci_get("xmodem.main.at_timeout_model", val, sizeof(val)) && atoi(val) > 0)
		g.at_timeout_model = atoi(val);
	if (!uci_get("xmodem.main.add_retry_delay", val, sizeof(val)) && atoi(val) >= 0)
		g.add_retry_delay = atoi(val);
	if (!uci_get("xmodem.main.add_retry_max", val, sizeof(val)) && atoi(val) >= 0)
		g.add_retry_max = atoi(val);
	if (!uci_get("xmodem.main.scan_log_level", val, sizeof(val)) && val[0])
		g.log_level = parse_log_level(val);
	env_level = getenv("XMODEM_SCAN_LOG_LEVEL");
...
int main(int argc, char **argv)
{
	...
	g.support_json = json_object_from_file(XMODEM_SUPPORT_JSON);
	g.port_rule_json = json_object_from_file(XMODEM_PORT_RULE_JSON);
	...
	sockfd = setup_socket();
	if (sockfd < 0) {
		syslog(LOG_ERR, "failed to setup socket %s: %s", XMODEM_SCAND_SOCKET, strerror(errno));
		return 1;
	}
